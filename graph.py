...
class graph():
    ...
    def build_from_mdp(self, mdp):
        ##turn transitions to weights by W=-log(P)
        if not isinstance(mdp.P, np.ndarray):
            if isinstance(mdp.P, list):
                mdp.P = np.array(mdp.P)
            else:
                mdp.P = mdp.P.todense()
        assert mdp.P.shape == (len(mdp.S), len(mdp.S))
    
        W = - np.log(mdp.P.copy())
        assert W.shape == (len(mdp.S), len(mdp.S))
        #print(W)

        self.num_V = len(mdp.S)
        ##init all the vertices
        for s in mdp.S:
            self.V.append(vertex(s, np.reshape(W[s,:], (len(mdp.S), 1))))
            ##init all the edges
            self.E.append(list())
            for s_ in mdp.S:
                self.E[-1].append(edge(s, s_, W))	
    ...
    def dijkstra(self):		
        self.P[self.start] = path(self.start, self.start)
        ...
        while True:
            ...
            while v_ != self.start:
                ...
                self.P[end].add(prev)
                v_ = prev
            self.P[end].list = self.P[end].list[-1::-1]
        ...
        if len(self.P.keys()) < len(self.V):
            for end in range(len(self.V)):
                if self.V[end].token is not True:
                    self.P[end] = path(self.start, end)
                    self.P[end].len = float('inf')
                    self.P[end].list = None
